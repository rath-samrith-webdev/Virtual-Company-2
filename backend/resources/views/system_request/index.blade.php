                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    @if($request->user->profile)
                                        <img class="inline-block h-10 w-10 rounded-full ring-2 ring-white" src="/images/profiles/user-{{$request->user->first_name}}/{{ $request->user->profile }}" alt="" />
                                    @else
                                        <span class="inline-flex bg-gray-500 items-center justify-center h-10 w-10 rounded-full ring-2 ring-white">{{$request->user->first_name[0]}}{{$request->user->last_name[0]}}</span>
                                    @endif
                                    {{$request->user->name}}
                                </th>
                                <td class="px-6 py-4">
                                ...
                                <td class="py-4 px-4 border-b justify-start border-grey-light text-right">
                                    @can('Rate edit')
                                        <a href="{{route('admin.system-requests.edit',$request->id)}}" class="text-grey-lighter font-bold py-1 rounded text-xs bg-green hover:bg-green-dark text-blue-400">Edit</a>
                                    @endcan
                                    @can('Rate delete')
                                        <form action="{{ route('admin.system-requests.destroy', $request->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('delete')
