    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
      <div class="container mx-auto px-6 py-1 pb-16">
          <div class="bg-white rounded my-6 p-5" style="background-color: #FCB22D">
              <h1 class="text-center font-bold">Edit Appointment</h1>
          </div>
        <div class="bg-white shadow-md rounded my-6 p-5">
          <form method="POST" action="{{ route('admin.appointments.update',$data['appointment']->id)}}">
            @csrf
            @method('put')
            <div class="flex flex-col space-y-2">
              <label for="title" class="text-gray-700 select-none font-medium">Title</label>
              <input id="title" type="text" name="name" value="{{ old('title',$data['appointment']->title) }}" placeholder="Appointment Title" class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
            </div>
            <div class="flex flex-col space-y-2">
              <label for="description" class="text-gray-700 select-none font-medium">Information</label>
                <div class="flex gap-2">
                    <select class="border border-gray-300  text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none" name="hospital_id" >
                        ...
                        @foreach($data['hospitals'] as $user)
                            <option value="{{$user->id}}" @if($user->id==$data['appointment']->hospital_id) selected  @endif>{{$user->name}}</option>
                        @endforeach
                    </select>
                    <div class="relative h-10">
                        <input
                            id="date-picker"
                            value="{{$data['appointment']->appointment_date}}"
                ...
                <div>
                    <h2 class="text-start">Appointment Maker</h2>
                    ...
                    <select class="border border-gray-300 text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none" name="user_id" @if(auth()->id()!==1)disabled @endif>
                        <option value="" disabled>Select Category</option>
                        @foreach($data['users'] as $user)
                            <option value="{{$user->id}}" @if($data['appointment']->user_id==$user->id) selected  @endif>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
