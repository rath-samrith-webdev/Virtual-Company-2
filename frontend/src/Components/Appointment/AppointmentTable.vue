    ...
    <el-table v-if="showTable" :data="tableData" height="450" style="width: 100%" class="mt-3">
      ...
      <el-table-column prop="status" label="Status">
        <template #default="scope">
          <el-tag v-if="scope.row.status=='Canceled'" type="primary">{{ scope.row.status }}</el-tag>
          <el-tag v-if="scope.row.status=='Confirmed'"  type="success">{{ scope.row.status }}</el-tag>
          <el-tag v-if="scope.row.status=='Pending'" type="info">{{ scope.row.status }}</el-tag>
          <el-tag v-if="scope.row.status=='Missing'" type="warning">{{ scope.row.status }}</el-tag>
          <el-tag v-if="scope.row.status=='Rejected'" type="danger">{{ scope.row.status }}</el-tag>
        </template>
      </el-table-column>
